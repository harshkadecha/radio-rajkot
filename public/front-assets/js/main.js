(function() {
  ...
  `;
    ...
    function initMapSingle() {
        ...
        const locations = [
            { lat: 23.0518058, lng: 72.4801875 },
        ]

        const contentString = `<div class="mapItem">
            D-506 Swati clover,<br />
            Besides shilp the address,<br />
            Near shilaj cross roads,<br />
            Shilaj,<br />
            Ahmedabad-380059,<br />
            Gujarat, India
        </div>`;
        ...
        const markers = locations.map((location) => {
            const marker = new HTMLMapMarker({
                latlng: location,
                map: map,
                html: `<div class="mapMarker flex-center bg-white rounded-100 bg-dark-1 size-40">
    ...
    function beachVacationSlider() {
        const sectionSlider = document.querySelectorAll('#beachVacation');
        if (!sectionSlider.length) return;
    ...
    function weekendGetawaysSlider() {
        const sectionSlider = document.querySelectorAll('#weekendGetaways');
        if (!sectionSlider.length) return;
